...
VERSION HISTORY:
v1 (current): Standalone Meetup recipe with v2 state machine (19 states), checkpoint/resume, image generation
...
- Meetup has anti-bot detection - requires 2.0s delays between all actions
...
- Anti-bot detection can cause failures if delays are too short
"""
...
# ============================================================================
# Platform Configuration (Meetup-specific)
# ============================================================================

PLATFORM = "meetup"
# HARDCODED: Coffee Code Philly Accelerator group URL
...
INTER_STEP_DELAY = 2.0  # Anti-bot delay - critical for Meetup
...
def log(tenant: str, state: str, message: str) -> None:
    ...
    print(f"[{ts}] [{tenant}] [meetup] [{state}] {message}")
...
# ============================================================================
# State Definitions (19 states for v2)
# ============================================================================
...
# State flow (simplified - skipping advanced feature states for initial implementation)
STATE_FLOW = [
    EventState.INIT,
...
STATE_CONFIG = {
    ...
    EventState.NAVIGATE: {"max_retries": 2, "base_delay": 2.0},  # Longer delays for Meetup
    EventState.AUTH_CHECK: {"max_retries": 1, "base_delay": 1.0},
    EventState.FILL_TITLE: {"max_retries": 2, "base_delay": 2.0},
    EventState.FILL_DATE: {"max_retries": 2, "base_delay": 2.0},
    EventState.FILL_TIME: {"max_retries": 2, "base_delay": 2.0},
    EventState.FILL_LOCATION: {"max_retries": 2, "base_delay": 2.0},
    EventState.FILL_DESCRIPTION: {"max_retries": 2, "base_delay": 2.0},
    EventState.VERIFY_FORM: {"max_retries": 2, "base_delay": 2.0},
    EventState.SUBMIT: {"max_retries": 1, "base_delay": 2.0},
    EventState.POST_SUBMIT: {"max_retries": 1, "base_delay": 2.0},
    EventState.VERIFY_SUCCESS: {"max_retries": 3, "base_delay": 2.0, "max_delay": 15.0},
}
...
class SessionManager:
    ...
    def _get_path(self, tenant_id: str) -> Path:
        ...
        return self.storage_dir / f"{safe_tenant}_meetup.json"
...
class CheckpointManager:
    ...
    def _get_path(self, tenant_id: str) -> Path:
        safe_tenant = tenant_id.replace("/", "_").replace(":", "_")
        return self.storage_dir / f"checkpoint_{safe_tenant}_meetup.json"
...
def check_success_url(url: str) -> bool:
    ...
    url_lower = url.lower()
    has_success = all(pattern in url_lower for pattern in SUCCESS_PATTERNS)
...
def browser_fetch(format: str = "markdown") -> tuple[str, str, str | None]:
    ...
    result, error = run_composio_tool("BROWSER_TOOL_FETCH_WEBPAGE", {
        "format": format,
        "wait": 2000,  # Longer wait for Meetup
    })
    if error:
        return "", "", error
    data = extract_data(result)
...
def generate_event_image(event_data: dict, custom_prompt: str | None = None) -> tuple[str | None, str | None]:
    ...
    if custom_prompt:
        ...
        prompt = (
            f"Create a modern, eye-catching event promotional graphic for: {event_data['title']}. "
            f"Style: professional, community-focused, suitable for Meetup. "
...
def handle_navigate(tenant_id: str, session: TenantSession) -> tuple[bool, EventState | None, str | None]:
    ...
    # Extra delay for Meetup anti-bot
...
def handle_fill_field(tenant_id: str, state: EventState, event_data: dict) -> tuple[bool, EventState | None, str | None]:
    ...
    # Build prompt based on field (Meetup-specific)
    prompts = {
        EventState.FILL_TITLE: f"Find the event title or event name field, click on it, clear any existing text, type exactly: {value}, then click outside",
        EventState.FILL_DATE: f"Find and click the date field or date picker, select: {value}. May need to navigate the calendar.",
        EventState.FILL_TIME: f"Find the start time field, click it, and enter: {value}",
        EventState.FILL_LOCATION: f"Find the venue or location field, click it, type: {value}, and select from suggestions or press Enter",
        EventState.FILL_DESCRIPTION: f"Find the description or 'about this event' field, click it, and type: {value}",
    }
    ...
    # Anti-bot delay - critical for Meetup
    time.sleep(INTER_STEP_DELAY)
...
def handle_submit(tenant_id: str) -> tuple[bool, EventState | None, str | None]:
    ...
    prompt = "Click the 'Publish' or 'Schedule Event' or 'Create Event' button. Wait for confirmation."
    success, error = browser_perform_task(prompt)

    if not success:
        return False, None, f"Submit failed: {error}"

    # Extra wait for Meetup processing
    time.sleep(3.0)
...
def handle_post_submit(tenant_id: str) -> tuple[bool, EventState | None, str | None]:
    ...
    # Meetup doesn't typically show modals after creation
    time.sleep(INTER_STEP_DELAY)
...
def handle_verify_success(tenant_id: str, event_data: dict) -> tuple[bool, EventState | None, str | None, str | None]:
    ...
    signals = {
        ...
        "confirmation": any(kw in content.lower() for kw in ["created", "published", "live", "scheduled"]),
    }
...
def run_workflow(tenant_id: str, event_data: dict, image_prompt: str | None = None,
    ...
    """Run the event creation workflow for Meetup.
    ...
    """
    log(tenant_id, "init", f"Starting Meetup workflow (resume={resume})")
    ...
    if current_state == EventState.PAUSED_2FA:
        result["resume_token"] = f"{tenant_id}:meetup:2fa"
        ...
        result["resume_token"] = f"{tenant_id}:meetup:auth"
        result["resume_instructions"] = (
            "Please log in to Meetup in the browser. "
...
print(f"[{datetime.utcnow().isoformat()}] Starting meetup_create_event recipe")
print(f"[{datetime.utcnow().isoformat()}] Using hardcoded group URL: {MEETUP_GROUP_URL}")
