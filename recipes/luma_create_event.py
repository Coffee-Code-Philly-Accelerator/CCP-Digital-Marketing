...
VERSION HISTORY:
v1 (current): Standalone Luma recipe with v2 state machine (19 states), checkpoint/resume, image generation
...
- Luma React date picker needs 1.5s extra wait time
...
- React date picker can be finicky - extra wait time helps
"""
...
# ============================================================================
# Platform Configuration (Luma-specific)
# ============================================================================
...
def log(tenant: str, state: str, message: str) -> None:
    ...
    print(f"[{ts}] [{tenant}] [luma] [{state}] {message}")
...
class SessionManager:
    ...
    def _get_path(self, tenant_id: str) -> Path:
        ...
        return self.storage_dir / f"{safe_tenant}_luma.json"
...
class CheckpointManager:
    ...
    def _get_path(self, tenant_id: str) -> Path:
        safe_tenant = tenant_id.replace("/", "_").replace(":", "_")
        return self.storage_dir / f"checkpoint_{safe_tenant}_luma.json"
...
def generate_event_image(event_data: dict, custom_prompt: str | None = None) -> tuple[str | None, str | None]:
    ...
    if custom_prompt:
        ...
        prompt = (
            ...
            f"Style: professional, vibrant colors, suitable for social media. "
...
def handle_fill_field(tenant_id: str, state: EventState, event_data: dict) -> tuple[bool, EventState | None, str | None]:
    ...
    prompts = {
        EventState.FILL_TITLE: f"Click on the event title field (may say 'Event Title' or 'Untitled Event'), clear any existing text, type exactly: {value}, then click outside the field to confirm",
        EventState.FILL_DATE: f"Click on the date field to open the date picker, select the date: {value}. Wait for the calendar to respond.",
        EventState.FILL_TIME: f"Click on the time field, enter the time: {value}",
        EventState.FILL_LOCATION: f"Click on the location or venue field, type: {value}, then select from the dropdown or press Enter",
        EventState.FILL_DESCRIPTION: f"Click on the description field (may say 'Add a description'), clear any existing text, and type: {value}",
    }
    ...
    # Post-step wait
    delay = DATE_PICKER_DELAY if state == EventState.FILL_DATE else INTER_STEP_DELAY
    time.sleep(delay)
...
def handle_submit(tenant_id: str) -> tuple[bool, EventState | None, str | None]:
    ...
    prompt = "Click the 'Publish' or 'Create Event' button to publish the event. Wait for the page to navigate to the new event."
    success, error = browser_perform_task(prompt)
...
def handle_post_submit(tenant_id: str) -> tuple[bool, EventState | None, str | None]:
    """Handle post-submit actions (modal dismissal)."""
...
def run_workflow(tenant_id: str, event_data: dict, image_prompt: str | None = None,
    ...
    """Run the event creation workflow for Luma.
    ...
    """
    log(tenant_id, "init", f"Starting Luma workflow (resume={resume})")
    ...
    if current_state == EventState.PAUSED_2FA:
        result["resume_token"] = f"{tenant_id}:luma:2fa"
        ...
        result["resume_token"] = f"{tenant_id}:luma:auth"
        result["resume_instructions"] = (
            "Please log in to Luma in the browser. "
...
print(f"[{datetime.utcnow().isoformat()}] Starting luma_create_event recipe")
