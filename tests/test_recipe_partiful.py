"""
Integration tests for the Partiful event creation recipe.
"""
...
RECIPE_FILE = str(RECIPES_DIR / "partiful_create_event.py")
...
class TestPartifulHyperbrowser:
    ...
    def test_with_profile_id(self, monkeypatch, base_env):
        base_env["CCP_PARTIFUL_PROFILE_ID"] = "partiful_profile_xyz"
        tool_calls = []
        ...
        assert start_call[1]["sessionOptions"]["profile"]["id"] == "partiful_profile_xyz"

    def test_custom_create_url(self, monkeypatch, base_env):
        base_env["partiful_create_url"] = "https://staging.partiful.com/create"
        tool_calls = []
        ...
        runpy.run_path(RECIPE_FILE)
        start_call = next(c for c in tool_calls if c[0] == "HYPERBROWSER_START_BROWSER_USE_TASK")
        assert "staging.partiful.com" in start_call[1]["task"]


class TestPartifulBrowserTool:
...
class TestPartifulEdgeCases:
    ...
    def test_success_url_pattern_in_output(self, monkeypatch, base_env):
        output = _run_recipe(monkeypatch, base_env)
        assert "partiful.com/e/" in output["success_url_pattern"]
