"""
Integration tests for the Luma event creation recipe.

Uses runpy.run_path() with mock builtins to execute the recipe in a
controlled environment, verifying the output dict.
"""
...
RECIPE_FILE = str(RECIPES_DIR / "luma_create_event.py")


def _run_recipe(monkeypatch, env_vars, tool_responses=None):
    """
    Execute the Luma recipe with controlled env vars and mocked runtime functions.

    Args:
        monkeypatch: pytest monkeypatch fixture
        env_vars: dict of environment variables to set
        tool_responses: dict mapping tool names to (result, error) tuples
    """
...
class TestLumaHyperbrowser:
    ...
    def test_with_profile_id(self, monkeypatch, base_env):
        ...
        assert output["task_id"] == "hb_job_2"

        # Verify profile was included in session options
        start_call = next(c for c in tool_calls if c[0] == "HYPERBROWSER_START_BROWSER_USE_TASK")
        assert start_call[1]["sessionOptions"]["profile"]["id"] == "profile_abc"


class TestLumaBrowserTool:
    def test_browser_tool_fallback(self, monkeypatch, base_env):
        ...
        assert output["session_id"] == "bt_sess_1"
        assert output["live_url"] == "https://live.example.com/bt"
        assert output["poll_tool"] == "BROWSER_TOOL_WATCH_TASK"
        assert output["poll_args_key"] == "taskId"


class TestLumaEdgeCases:
    ...
    def test_tool_error_raises(self, monkeypatch, base_env):
        ...
        def mock_tool(tool_name, arguments):
            return (None, "Browser service unavailable")

        monkeypatch.setattr(builtins, "run_composio_tool", mock_tool, raising=False)
        with pytest.raises(Exception, match="Failed to create browser task"):
            runpy.run_path(RECIPE_FILE)

    def test_no_task_id_raises(self, monkeypatch, base_env):
        for key, val in base_env.items():
            monkeypatch.setenv(key, val)

        def mock_tool(tool_name, arguments):
            if tool_name == "HYPERBROWSER_START_BROWSER_USE_TASK":
                return ({"data": {"data": {}}}, None)  # No jobId
            return ({"data": {}}, None)

        monkeypatch.setattr(builtins, "run_composio_tool", mock_tool, raising=False)
        with pytest.raises(Exception, match="No task ID returned"):
            runpy.run_path(RECIPE_FILE)

    def test_image_url_included_in_output(self, monkeypatch, base_env):
        base_env["event_image_url"] = "https://img.example.com/photo.jpg"
        output = _run_recipe(monkeypatch, base_env)
        assert output["event_image_url"] == "https://img.example.com/photo.jpg"
