...
gui/src-tauri/src/config.rs  # API keys and configuration
gui/src-tauri/src/db.rs      # Telemetry database
```
...
```bash
# Create PR for both remotes (Coffee-Code-Philly-Accelerator/CCP-Digital-Marketing and Coffee-Code-Philly-Accelerator/CCP-Digital-Marketing)
gh pr create \
  --repo Coffee-Code-Philly-Accelerator/CCP-Digital-Marketing \
...
echo "x=1" >> scripts/test_scanner.py  # Missing spaces around =
git add scripts/test_scanner.py
git commit -m "test: trigger formatting scanner"
git push

# Run scanner (no dry run)
gh workflow run autonomous-code-scanner.yml --repo Coffee-Code-Philly-Accelerator/CCP-Digital-Marketing
...
cat > scripts/test_security.py << 'EOF'
import subprocess
...
git add scripts/test_security.py
