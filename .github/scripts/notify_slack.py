...
# Configuration
SLACK_CHANNEL_ID = os.environ.get("SLACK_CHANNEL_ID", "REPLACE_WITH_CHANNEL_ID")


def build_commit_message(
    branch: str,
    commit_short_sha: str,  # commit_sha removed - only short version used
    author: str,
    committer: str,
    message_subject: str,
    message_body: str,
...
def build_scanner_message(
    ...
    return f"""## 🤖 CCP Digital Marketing Autonomous Code Scanner
...
def build_issue_fix_message(
    ...
    status_map = {
        "success": ("OK", "PR CREATED"),
        "no-changes": ("SKIP", "NO CHANGES"),
        "failure": ("FAIL", "FAILURE"),
        "skipped": ("SKIP", "SKIPPED"),
    }
    _, label = status_map.get(status, ("?", status.upper()))
...
def build_docs_update_message(
    ...
    # Format affected docs as bullet list
    if affected_docs:
        docs_lines = "\n".join(
...
def send_slack_notification(message: str) -> bool:
    ...
    # Call Composio API directly (same as Rube MCP)
    try:
        import requests

        print(f"Sending notification to Slack channel {SLACK_CHANNEL_ID}...")

        # Use v2 actions execute endpoint with entityId (Rube MCP approach)
        api_url = (
        ...
        # Use actual entity ID from Composio dashboard
        payload = {
            "entityId": os.environ.get("SLACK_CONNECTED_ACCOUNT_ID", "default"),
            "appName": "slack",
            "input": {
...
def main():
    ...
    if notify_kind == "docs-update":
        ...
        # Scanner notification mode (existing logic)
        status = os.environ.get("WORKFLOW_STATUS", "unknown")
        prs_created = int(os.environ.get("PRS_CREATED", "0"))
        workflow_run_url = os.environ.get("WORKFLOW_RUN_URL", "")
        prs_url = os.environ.get(
            "PRS_URL",
            "https://github.com/Coffee-Code-Philly-Accelerator/CCP-Digital-Marketing/pulls?q=is%3Apr+label%3Ascanner",
        )

        # Parse PR details (comma-separated)
