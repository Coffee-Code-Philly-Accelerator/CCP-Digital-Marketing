...
PROTECTED_PATTERNS = [
    ...
    ".env",
    "recipes/",
    "gui/src-tauri/src/composio.rs",
    "gui/src-tauri/src/config.rs",
    "gui/src-tauri/src/draft.rs",
]
...
def find_def_end_line(lines: list[str], start_idx: int) -> int:
    ...
    Returns the 0-based index of the last line of the def statement.
    """
    depth = 0
    for i in range(start_idx, len(lines)):
        ...
        if depth <= 0 and ":" in line:
            # Find the colon that ends the signature (not inside strings)
            stripped = line.rstrip()
            if stripped.endswith(":"):
                return i
            # Handle colon followed by inline body (rare but possible)
...
def normalize_import(imp: str) -> list[str]:
    """Normalize an import statement, expanding combined imports.

    e.g. 'from typing import List, Optional' stays as-is.
    Returns a list with one element normally.
    """
    return [imp.strip()]
...
def apply_type_hints(suggestions_path: str) -> dict:
    ...
    for file_path, file_funcs in by_file.items():
        ...
        # Find actual line numbers via AST, sort bottom-up
        located = []
        ...
        # Sort bottom-up (descending line) to avoid drift
        ...
        for actual_line, func in located:
            ...
            start_idx = actual_line - 1  # Convert to 0-based
            if start_idx >= len(lines) or start_idx < 0:
                continue

            # Find the end of the current def line(s)
            ...
            # Ensure typed signature has matching async prefix
            ...
            # Ensure the typed signature ends with ':'
        ...
        # Collect and insert missing imports
        ...
        if imports_to_add:
            ...
            if insert_idx >= 0:
                ...
                # No imports exist, insert at top (after any module docstring/comments)
                insert_point = 0
                for i, line in enumerate(lines):
                    stripped = line.strip()
                    if (
