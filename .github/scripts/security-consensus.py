...
{{
  "findings_validated": [
    {{
      ...
      "risk_level": "high",
      "recommended_fix": "Use parameterized queries instead of string concatenation: cursor.execute('SELECT * FROM users WHERE id = ?', (user_id,))",
      "justification": "String concatenation in SQL query creates SQL injection vulnerability. User-controlled input flows directly into query without sanitization."
    }}
  ],
  "summary": {{
    "total_reviewed": 5,
...
def run_consensus(findings: list[dict[str, Any]], api_key: str) -> dict[str, Any]:
    ...
    try:
        response = client.messages.create(
            ...
            max_tokens=16000,
            temperature=0,  # Deterministic for security validation
            thinking={
                "type": "enabled",
                "budget_tokens": 10000,  # Extended thinking for security analysis
            },
        ...
        print(f"::error::Unexpected error during consensus: {e}", file=sys.stderr)
        sys.exit(1)
...
def main():
    ...
    # Get inputs from command line or environment
    findings_path = Path(
    ...
    if metadata.get("input_tokens"):
        ...
        print(f"\n::set-output name=consensus_cost::{cost}")
